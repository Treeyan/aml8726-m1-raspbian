...
struct sd_thread_helper_struct {
    struct workqueue_struct *wqs;
    struct work_struct       wks;
    void   *                 pal;
};

static void card_thread_helper( struct work_struct * wks )
{
    struct sd_thread_helper_struct  *   sdt;
    struct card_host                *   host;

    sdt     = container_of( wks, struct sd_thread_helper_struct, wks );
    host    = sdt->pal;
    
    host->card_task = kthread_run( card_reader_monitor, sdt, "card" );

    if (!host->card_task) {	
        // ddd, BUG: FIXME
        // sdt->wqs not be destroy, destroy_workqueue( sdt->wqs );
        kfree( sdt );
...
{	
    sdio_host = host;
	host->dma_buf = dma_alloc_coherent( NULL, 
                                        host->max_req_size, 
                                        (dma_addr_t *)&host->dma_phy_buf, 
                                        GFP_KERNEL );
...
// ddd
    {
        ...
        sdt = kmalloc( sizeof( *sdt ), GFP_ATOMIC );
        if( !sdt ) {
            printk( "Failed to alloc work_struct\n" );
            return -ENOMEM;
        }

        sdt->wqs = create_workqueue( "mmc-sd" );
        if( !sdt->wqs ) {
            kfree( sdt );
            return -ENOMEM;
        }
        ...
        INIT_WORK( &sdt->wks, card_thread_helper );
        queue_work( sdt->wqs, &sdt->wks );
    }
...
static int card_reader_monitor(void *data)
{
    ...
    flush_workqueue  ( sdt->wqs );
    destroy_workqueue( sdt->wqs );
    kfree( sdt );
...
static int amlogic_card_probe(struct platform_device *pdev) 
{
    ...
    printk( "Amlogic card_probe enter.\n" );
