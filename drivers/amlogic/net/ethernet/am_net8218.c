...
static void aml_set_rmii_mode( struct am_net_private * np )
{
    unsigned long   val;
    int             k;

    val = (1 << 14) | (7 << 5) | np->phy_addr;
    mdio_write( np->mii, np->phy_addr, 18, val );
    
    val = BMCR_RESET;
    mdio_write( np->mii, np->phy_addr, MII_BMCR, val);
    //waiting to phy reset ok....
    for (k = 0; (mdio_read( np->mii, np->phy_addr, MII_BMCR )) & (BMCR_RESET)
            && k < 1000; k++) {
        udelay(1);
    }
