...
static int dwc_ahci_set_lpm( struct ata_link    *   ap,
...
static int dwc_ahci_stop_fis_rx(struct ata_port *ap)
{
	...
	tmp = ata_wait_register( ap,
                             port_mmio + PORT_CMD, PORT_CMD_FIS_ON,
                             PORT_CMD_FIS_ON, 
                             10, 
                             1000 );
...
static int dwc_ahci_set_lpm( struct ata_link   *    link,
                             enum ata_lpm_policy    policy,
                             unsigned               hints )
{
...
//	struct ahci_host_priv *hpriv = host->private_data;
	...
	if (!dwc_ahci_skip_host_reset) {
		 ...
		 */
		tmp = ata_wait_register( host->ports[0] ,
                                 mmio + HOST_CTL,
                                 HOST_RESET,
                                 HOST_RESET, 
                                 10, 
