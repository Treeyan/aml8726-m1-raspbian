...
#if defined( CONFIG_MAGIC_SYSRQ ) && defined( CONFIG_SERIAL_MESON_ONE_CONSOLE )
...
#define AMLOGIC_DEV_LEGACY  -1
#define AMLOGIC_UART_NAME   "ttyS"
#define AMUART_DRV_NAME     "am_uart"
#define UART_NR	2
...
#define UART_A_PORT         0
#define UART_B_PORT         1
...
//#endif
#define in_l(addr)	            readl((void*)(addr))        //  inl((unsigned long )addr)
#define out_l(v,addr)	        writel((v),(void*)(addr))   //outl(v,(unsigned long )addr)
        ...
        unsigned short rx)
{
    ...
    struct tty_port *tty_port = &info->port.state->port;
...
static void BH_receive_chars(struct am_uart_port *info)
{
    ...
    tty_port->low_latency = 1;	// Originally added by Sameer, serial I/O slow without this
    flag = TTY_NORMAL;
...
static int am_uart_get_poll_char(struct uart_port *port)
{
    ...
    /*
       ...
       rx = 0;
     */
    while (!(status = __raw_readl(&uart->status) & 0x7f));
    rx = __raw_readl(&uart->rdata);
    mutex_unlock(&info->info_mutex);
...
static void am_uart_set_ldisc(struct uart_port *port, struct ktermios * new )
{
#ifdef PRINT_DEBUG
    struct am_uart_port * info = &am_ports[port->line];
    if(info->line == 0)
...
static void am_uart_pm(struct uart_port *port, unsigned int state,
...
#ifdef CONFIG_SERIAL_MESON_ONE_CONSOLE

static int __init am_uart_console_setup(struct console *co, char *options)
//static int am_uart_console_setup(struct console *co, char *options)
{
    struct clk *sysclk;
...
#else // !CONFIG_SERIAL_MESON_ONE_CONSOLE

#define AM_UART_CONSOLE NULL

#endif // CONFIG_SERIAL_MESON_ONE_CONSOLE

struct uart_driver am_uart_reg = {
    .owner			= THIS_MODULE,
    .driver_name	= "serial",
//    .dev_name		= "ttyS",
    .dev_name       = AMLOGIC_UART_NAME,
    .major			= 4,
    .minor			= 64,
    .nr             = 2,
...
static int am_uart_probe(struct platform_device *pdev)
{
    //
    //    Nothing to do here.
    //
    ...
    for(i=0; i<UART_NR; i++)
    {
        ...
        printk( "port-addr=%p sizeof=%d line-addr=%p port-line=%d\n", 
                port,
                sizeof( *port ),
                &port->line,
                port->line );
...
static int am_uart_remove(struct platform_device *pdev)
{
    struct am_uart_port *up;
    int i,ret = 0;
...
struct platform_driver am_uart_isa_driver = {
    ...
    .remove		= am_uart_remove,
    .suspend	= am_uart_suspend,
    .resume		= am_uart_resume,
    .driver		= {
        .name	= AMUART_DRV_NAME, //"am_uart",
        .owner	= THIS_MODULE,
    },
};
